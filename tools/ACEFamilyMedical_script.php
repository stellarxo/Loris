...
$result = $db->pselect("SELECT m.a_aut_spect_disorder, m.a_aut_spect_disorder_who,
                        ...
                        WHERE s.ID = 482 AND tflag.CommentID NOT LIKE 'DDE%' AND mflag.CommentID NOT LIKE 'DDE%'", array()) ;
...
foreach($result as $row) {
    foreach($mapping as $key=>$val) {
        if ($row[$key] == '1_yes') { print "here";
            if (strpos($row[$key."_who"], 'parents_mom') !== false) { print "mom";
