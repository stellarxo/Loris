...
if(!empty($_REQUEST['sessionID']) && is_numeric($_REQUEST['sessionID'])) {
    ...
    if(isset($_REQUEST['filter'])) {
        ...
        function addFilter($fname) {
            global $DB;
            global $filter;
            global $formToFilter;

            if(!empty($filter[$fname])) {
                // Dave: copied from original when I turned this into a
                //       function from a million if statements, but this 
                //       should be changed. 
                //       It's vulnerable to an SQL injection attack.

                // override to allow for comma separated list 
                // if input contains a comma.
                if(!ereg(",", $filter[$fname])) {
                    return ' AND ' . $formToFilter[$fname] . ' LIKE ' . $DB->quote($filter[$fname] . '%');
                }
                return ' AND '.  $formToFilter[$fname] . ' IN (' . $filter[$fname] .')';
            }
            return '';
        }
        
        foreach($formToFilter as $varname=> $field) {
            $extra_where_string .= addFilter($varname);
        }
        
        
        if(is_array($filter['order'])) {
            if($filter['order']['field'] != '' && $headers[$filter['order']['field']] != '')
                $extra_orderby = $headers[$filter['order']['field']] . ' ' . $filter['order']['asc'] . ',';
            else
                $extra_orderby = '';
        }
        ...
        if(PEAR::isError($visit_tpl_data['timepoints']))  {
            print $visit_tpl_data['timepoints']->getMessage()."<br>\n";
        }
    }
